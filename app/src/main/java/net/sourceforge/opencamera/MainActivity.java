...
import net.sourceforge.opencamera.sensorlogging.GpsInfo;
...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        int permissionCheckStorage = ContextCompat.checkSelfPermission(MainActivity.this,
        ...
        if (MyDebug.LOG) {
            ...
            Log.d(TAG, "Created GpsInfo object");
        }
    ...
    public GpsInfo getGpsInfoManager() {
    ...
    private void onCreateInner(Bundle savedInstanceState) {
        ...
        prefEditor.putBoolean(PreferenceKeys.GpsPreferenceKey, true);
        prefEditor.apply();
        ...
        if (MyDebug.LOG)
            Log.d(TAG, "onCreate: time after setting immersive mode listener: "
                    + (System.currentTimeMillis() - debug_time));
            ...
            if (version_code != -1) {
                ...
                if (whats_new_enabled) {
                    ...
                    if (has_done_first_time && allow_show_whats_new
                        ...
                        alertDialog.setPositiveButton(android.R.string.ok, null);
                        /*
                         * alertDialog.setNegativeButton(R.string.donate, new
                         ...
                         * if( MyDebug.LOG )
                         * Log.d(TAG, "donate");
                         * // if we change this, remember that any page linked to must abide by Google
     ...
     * - For the current camera ID, there is only one camera with the same
     * front/back/external
    ...
    void setDeviceDefaults() {
        ...
        if (is_samsung || is_oneplus) {
            ...
            // update for v1.37: significant improvements have been made for standard flash
            // and Camera2 API. But OnePlus 3T still has problem
            // that photos come out with a blue tinge if flash is on, and the scene is
            // bright enough not to need it; Samsung devices also seem
            // to work okay, testing on S7 on RTL, but still keeping the fake flash mode in
            // place for these devices, until we're sure of good
            // behaviour
            // update for testing on Galaxy S10e: still needs fake flash
            // has also been reported to me that OnePlus 8 and 8 Pro have problems with
            // flash on Camera2 API unless fake flash enabled
            if (MyDebug.LOG)
    ...
    protected void onDestroy() {
        ...
        // otherwise we can have crash if we need Renderscript after calling
        // releaseAllContexts(), or because rs has been set to
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && activity_count == 0) {
            // See note in HDRProcessor.onDestroy() - but from Android M, renderscript
            // contexts are released with releaseAllContexts()
            // doc for releaseAllContexts() says "If no contexts have been created this
            // function does nothing"
            // Important to only do so if no other activities are running (see
            // activity_count). Otherwise risk
            // of crashes if one activity is destroyed when another instance is still using
            // Renderscript. I've
            // been unable to reproduce this, though such RSInvalidStateException crashes
            // from Google Play.
            if (MyDebug.LOG)
     ...
     * Allows using the Mi Band 2 as a Bluetooth remote for Open Camera to take
     * photos or start/stop
     * videos.
     */
    ...
    public void syncSettings() {
        ...
        if (!softwareSyncController.isSettingsBroadcasting()) {
            ...
            applicationInterface.getSoftwareSyncUtils().broadcastSettings(settings); // leader's settings are locked
                                                                                     // here as well
    ...
    public int getNextMultiCameraId() {
        ...
        if (indx == -1) {
            ...
            // this shouldn't happen, but if it does, revert to the first camera id in the
            // set
    ...
    public void lockWhiteBalance(View view) {
        ...
        preview.showToast(white_balance_lock_toast,
                preview.isWhiteBalanceLocked() ? R.string.white_balance_locked : R.string.white_balance_unlocked);
    }
    ...
    public void openSettings() {
        ...
        bundle.putBoolean(PreferenceKeys.SupportsMagnetometerKey,
                mRawSensorInfo.isSensorAvailable(Sensor.TYPE_MAGNETIC_FIELD));
        ...
        if (video_quality != null && this.preview.getCameraController() != null) {
            ...
            String video_quality_preference_key = PreferenceKeys
                    .getVideoQualityPreferenceKey(this.preview.getCameraId(), is_high_speed);
            if (MyDebug.LOG)
        ...
        if (this.preview.getCameraController() != null) {
            bundle.putBoolean(PreferenceKeys.SupportsVideoImuSync,
                    this.preview.getCameraController().supportsVideoImuSync());
        }
    ...
    public boolean usingKitKatImmersiveMode() {
        // whether we are using a Kit Kat style immersive mode (either hiding GUI, or
        // everything)
    ...
    public void setWindowFlagsForCamera() {
        ...
        if (preview != null) {
            // also need to call setCameraDisplayOrientation, as this handles if the user
            // switched from portrait to reverse landscape whilst in settings/etc
    ...
    private Bitmap loadThumbnailFromUri(Uri uri, int sample_factor, boolean mediastore) {
        ...
        try {
            ...
            if (!mediastore) {
                // When loading from a mediastore, the bitmap already seems to have the correct
                // orientation.
                // But when loading from a saf uri, we need to apply the rotation.
                // E.g., test on Galaxy S10e with ghost image last image option, when using SAF,
                // in portrait orientation, after pause/resume.
                        ...
                        try {
                            if (!media.mediastore) {
                                if (media.video) {
                                    ...
                                    ParcelFileDescriptor pfd_saf = null; // keep a reference to this as long as
                                                                         // retriever, to avoid risk of pfd_saf being
                                                                         // garbage collected
    ...
    private void createImageSavingNotification() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            Notification.Builder builder = new Notification.Builder(this, CHANNEL_ID)
                    ...
                    .setContentText(getString(R.string.image_saving_notification) + " " + n_images_to_save + " "
    ...
    private void openGallery() {
        ...
        boolean is_raw = false; // note that getLastMediaScanned() will never return RAW images, as we only
                                // record JPEGs
        if (uri == null) {
            if (MyDebug.LOG)
    ...
    void cameraSetup() {
        ...
        // MainUI.showGUI()
        // However still nee to update visibility of icons where visibility depends on
        ...
        View exposureButton = findViewById(R.id.exposure);
        {
            ...
            exposureButton.setVisibility(supportsExposureButton() && !mainUI.inImmersiveMode() &&
                    !(applicationInterface.isSoftwareSyncRunning() && softwareSyncController.isLeader()
                            && softwareSyncController.isSettingsBroadcasting()) ? View.VISIBLE : View.GONE);
        }
    ...
    private void setManualFocusSeekbar(final boolean is_target_distance) {
        ...
        ManualSeekbars.setProgressSeekbarScaled(focusSeekBar, 0.0, preview.getMinimumFocusDistance(),
    ...
    public boolean showManualFocusSeekbar(final boolean is_target_distance) {
        ...
        if (is_target_distance) {
            is_visible = is_visible
                    && (applicationInterface.getPhotoMode() == MyApplicationInterface.PhotoMode.FocusBracketing)
                    && !preview.isVideo();
        }
        return is_visible;
    }
    ...
    public boolean supportsHDR() {
        ...
        return (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && large_heap_memory >= 128
    ...
    public boolean supportsPanorama() {
        ...
        return (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && large_heap_memory >= 256
                && applicationInterface.getGyroSensor().hasSensors());
    ...
    public boolean supportsNoiseReduction() {
        // require at least Android 5, for the Renderscript support in HDRProcessor, but
        // we require
    ...
    public boolean supportsPreviewBitmaps() {
        ...
        // even if in future we allow TextureView pre-Android 5, we still need Android
        // 5+ for Renderscript.
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && preview.getView() instanceof TextureView
                && large_heap_memory >= 128;
    }
    ...
    // if we change this, remember that any page linked to must abide by Google Play
    // developer policies!
    // public static final String DonateLink =
    // "https://play.google.com/store/apps/details?id=harman.mark.donation";
    ...
    private void showPhotoVideoToast(boolean always_show) {
        ...
        if (preview.isVideo()) {
            ...
            String capture_rate_string = (capture_rate < 9.5f) ? new DecimalFormat("#0.###").format(capture_rate)
                    : "" + (int) (profile.videoCaptureRate + 0.5);
