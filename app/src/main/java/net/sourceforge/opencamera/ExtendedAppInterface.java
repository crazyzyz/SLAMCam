...
/**
 * Extended implementation of ApplicationInterface, adds raw sensor recording
 * layer and RecSync
...
public class ExtendedAppInterface extends MyApplicationInterface {
    ...
    private final GpsInfo mGpsInfo;
    ...
    ExtendedAppInterface(MainActivity mainActivity, Bundle savedInstanceState) {
        ...
        mGpsInfo = mainActivity.getGpsInfoManager();
        mMainActivity = mainActivity;
        mFlashController = new FlashController(mainActivity);
        // We create it only once here (not during the video) as it is a costly
        // operation
     ...
     *
     * @return a {@link Pair} describing the current phase error if RecSync is
     *         running, or null
     *         otherwise.
     */
    ...
    public void startingVideo() {
        ...
        if (mPrefs.isIMURecordingEnabled() && useCamera2()
                && (mPrefs.isGyroEnabled() || mPrefs.isAccelEnabled() || mPrefs.isMagneticEnabled())) {
        ...
        } else if (mPrefs.isIMURecordingEnabled()
                && !(mPrefs.isGyroEnabled() || mPrefs.isMagneticEnabled() || mPrefs.isAccelEnabled())) {
            mMainActivity.getPreview().showToast(null, "Requested IMU recording but no sensors were enabled");
            mMainActivity.getPreview().stopVideo(false);
        }

        // Start GPS recording if enabled and IMU recording is enabled
        if (mPrefs.isIMURecordingEnabled() && mPrefs.isGpsEnabled() && useCamera2()) {
            if (mGpsInfo.enableGps()) {
                mGpsInfo.startRecording(mMainActivity, mLastVideoDate);
                if (MyDebug.LOG) {
                    Log.d(TAG, "GPS recording started");
                }
            } else {
                if (MyDebug.LOG) {
                    Log.e(TAG, "Failed to enable GPS");
                }
                mMainActivity.getPreview().showToast(null, "GPS unavailable or no permission");
            }
        }
    ...
    public void stoppingVideo() {
        ...
        // Stop GPS recording if it was enabled
        if (mGpsInfo.isRecording()) {
            mGpsInfo.stopRecording();
            mGpsInfo.disableGps();
            if (MyDebug.LOG) {
    ...
    public void startImu(boolean wantAccel, boolean wantGyro, boolean wantMagnetic, Date currentDate) {
        ...
        // mRawSensorInfo.startRecording(mMainActivity, mLastVideoDate, get Pref(),
        // getAccelPref())
    ...
    /**
     * Starts RecSync by instantiating a {@link SoftwareSyncController}. If one is
     * already running
    ...
    public void startSoftwareSync() {
        ...
        try {
            mSoftwareSyncController = new SoftwareSyncController(mMainActivity, mPhaseAlignController,
                    mPeriodCalculator);
        ...
        if (mSoftwareSyncController.isLeader()) {
            ...
            String action;
            WifiManager wifiManager = (WifiManager) mMainActivity.getApplicationContext()
    ...
    private class HotspotStatusChecker extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (state != WIFI_AP_STATE_ENABLED) {
                ...
                disableRecSyncSetting();
                showSimpleAlert("Hotspot was stopped",
                        "Stopping RecSync. Enable either Wi-Fi or hotspot and re-enable RecSync in the settings.");
            }
        }
    }
    ...
    private class WifiStatusChecker extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (networkInfo == null || networkInfo.getType() != ConnectivityManager.TYPE_WIFI) {
                ...
                disableRecSyncSetting();
                showSimpleAlert("Wi-Fi was stopped",
                        "Stopping RecSync. Enable either Wi-Fi or hotspot and re-enable RecSync in the settings.");
            }
        }
    }
    ...
    /**
     * Whether SoftwareSync is currently running (i.e.
     * {@link SoftwareSyncController} is
     * initialized).
     *
